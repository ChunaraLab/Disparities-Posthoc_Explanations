...
FILENAMES = [
    'training_data_-2.7.csv',
    'training_data_-0.7.csv',
    'training_data_-0.5.csv',
    'training_data_0.5.csv',
    'training_data_1.5.csv',
    'training_data_2.7.csv'
]
...
def main(args):
    ...
    for fname in FILENAMES:
        ...
        test_data_fname = fname.replace('training_data', 'testing_data')
        test_data_path = BASE_PATH + test_data_fname
        print(f"Processing file: {train_data_path} and {test_data_path}")

        # Extract the percent removal from the filename
        percent_removal = fname.split('training_data_')[-1].replace('.csv', '')
        ...
        for trial in range(total_trials):
            ...
            # Set remove_interaction based on the value of args.setup
            remove_interaction = False
            if args.setup == 2:
                remove_interaction = True
            processor = DataProcessor(train_data_path, test_data_path, sensitive_attribute, setup= args.setup, group_attribute_c = True, remove_interaction=remove_interaction) #remove_interaction=True if you want to remove the LM interaction term, setup true 
            target = 'Y'
    ...
    # base_plot_path = "/Users/salman/Desktop/exp_qua/new_3_objective/dgp_final/plot_data/objective_2_final/plot_without_M"
